...
func TestPLATODBLoad(t *testing.T) {
	...
	// GDRatio is valid if FLAGS & 1<<3 is != 0
	//---------------------------------------------------------------------------
	...
	if drec.FLAGS&(1<<3) == 0 {
		t.Errorf("GD data should be valid on 2022-Dec-31, but FLAGS & 1<<3 = %d", drec.FLAGS&1<<3)
	}
	...
	if drec.FLAGS&(1<<3) != 0 {
		t.Errorf("GD data should NOT be valid on 2023-Jan-1, but FLAGS & 1<<3 = %d", drec.FLAGS&1<<3)
	}
}
