...
// -----------------------------------------------------------------------------------
type RatioFunc func(*newdata.EconometricsRecord, *newdata.EconometricsRecord) (float64, float64, float64)
...
//
//		                    |<---------------HOLD-WINDOW--------------->|
//		           <-- SELL |<-- HoldWindowNeg -->|<-- HoldWindowPos -->| BUY -->
//		 -------------------|---------------------|---------------------|------------------------
//		                                         0.0
//
// INPUTS
...
//		h1,h2  - h1 is HoldWindowNegative, h2 is HoldAreaPositive.
//		         the min and max values less than and greater than 0 to be considered as the
//	          "hold window".
//
// RETURNS
//
...
//	               data1,data2 - the data values found at t1 and t2
...
// 	f1 := p.MyInvestor().cfg.C1 + p.GetMetric()
// 	f2 := p.MyInvestor().cfg.C2 + p.GetMetric()
// 	flds := []string{f1, f2}

// 	rec1, err := db.Select(t1, flds)
// 	if err != nil {
...
// 	//rec := newdata.CSVDBFindLRecord(t)
// 	rec,err :=
// 	if rec == nil {
// 		err := fmt.Errorf("nildata: newdata.LinguisticDataRecord for %s not found", t.Format("1/2/2006"))
// 		return 0, err
// 	}
// 	c1val, err := newdata.GetLValue(rec, C1, datatype)
// 	if err != nil {
// 		log.Panicf("error getting Linguistic value: %s", err.Error())
// 	}
// 	c2val, err := newdata.GetLValue(rec, C2, datatype)
// 	if err != nil {
// 		log.Panicf("error getting Linguistic value: %s", err.Error())
// 	}
...
// 		return 0, err
// 	}
// 	c1val, err := newdata.GetLValue(rec, "ALL", datatype)
// 	if err != nil {
// 		log.Panicf("error getting Linguistic value: %s", err.Error())
// 	}
// 	return c1val, nil
...
// 		return 0, err
// 	}
// 	c1val, err := newdata.GetValueByColName(rec, colname)
// 	if err != nil {
