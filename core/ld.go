...
// LDInfluencer is the Influencer that predicts based on Linguistic Sentiment associated with a country
type LDInfluencer struct {
	cfg                 *util.AppConfig
	Delta1              int
	Delta2              int
...
// GetID - get ID string
func (p *LDInfluencer) GetID() string {
	return p.ID
}

// SetAppConfig - set cfg
func (p *LDInfluencer) SetAppConfig(cfg *util.AppConfig) {
	p.cfg = cfg
}

// GetDelta1 - get Delta1
func (p *LDInfluencer) GetDelta1() int {
	return p.Delta1
}

// SetDelta1 - set Delta1
func (p *LDInfluencer) SetDelta1(d int) {
	p.Delta1 = d
}

// GetDelta2 - get Delta2
func (p *LDInfluencer) GetDelta2() int {
	return p.Delta2
}

// SetDelta2 - set Delta2
func (p *LDInfluencer) SetDelta2(d int) {
	p.Delta2 = d
}

// SetID - set ID
func (p *LDInfluencer) SetID() {
	p.ID = fmt.Sprintf("LDInfluencer|%d|%d|%s", p.Delta1, p.Delta2, util.GenerateRefNo())
}

// Init - initializes a LDInfluencer
func (p *LDInfluencer) Init(i *Investor, cfg *util.AppConfig, delta4 int) {
	p.myInvestor = i
	p.cfg = cfg
	p.SetID()
	// p.flagpos = 5
...
func (p *LDInfluencer) Subclass() string {
	return "LDInfluencer"
}
...
// ----------------------------------------------------------------------------
func (p *LDInfluencer) DNA() string {
...
//	vaLD       - an indicator value at T3 - p.Delta2 days
...
//
// ---------------------------------------------------------------------------
func (p *LDInfluencer) GetPrediction(t3 time.Time) (string, float64, float64, float64, float64, error) {
...
// ------------------------------------------------------------------------------------
func (p *LDInfluencer) CalculateFitnessScore() float64 {
