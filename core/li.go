...
// LIInfluencer is the Influencer that predicts based on Linguistic Sentiment associated with a country
type LIInfluencer struct {
	cfg                 *util.AppConfig
	Delta1              int
	Delta2              int
...
// GetID - get ID string
func (p *LIInfluencer) GetID() string {
	return p.ID
}

// SetAppConfig - set cfg
func (p *LIInfluencer) SetAppConfig(cfg *util.AppConfig) {
	p.cfg = cfg
}

// GetDelta1 - get Delta1
func (p *LIInfluencer) GetDelta1() int {
	return p.Delta1
}

// SetDelta1 - set Delta1
func (p *LIInfluencer) SetDelta1(d int) {
	p.Delta1 = d
}

// GetDelta2 - get Delta2
func (p *LIInfluencer) GetDelta2() int {
	return p.Delta2
}

// SetDelta2 - set Delta2
func (p *LIInfluencer) SetDelta2(d int) {
	p.Delta2 = d
}

// SetID - set ID
func (p *LIInfluencer) SetID() {
	p.ID = fmt.Sprintf("LIInfluencer|%d|%d|%s", p.Delta1, p.Delta2, util.GenerateRefNo())
}

// Init - initializes a LIInfluencer
func (p *LIInfluencer) Init(i *Investor, cfg *util.AppConfig, delta4 int) {
	p.myInvestor = i
	p.cfg = cfg
	p.SetID()
	// p.flagpos = 5
...
func (p *LIInfluencer) Subclass() string {
	return "LIInfluencer"
}
...
// ----------------------------------------------------------------------------
func (p *LIInfluencer) DNA() string {
...
//	vaLI       - an indicator value at T3 - p.Delta2 days
...
//
// ---------------------------------------------------------------------------
func (p *LIInfluencer) GetPrediction(t3 time.Time) (string, float64, float64, float64, float64, error) {
...
// ------------------------------------------------------------------------------------
func (p *LIInfluencer) CalculateFitnessScore() float64 {
