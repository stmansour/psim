...
// L5Influencer is the Influencer that predicts based on Linguistic Sentiment associated with a country
type L5Influencer struct {
	cfg                 *util.AppConfig
	Delta1              int
	Delta2              int
	Delta4              int
...
// GetID - get ID string
func (p *L5Influencer) GetID() string {
	return p.ID
}

// SetAppConfig - set cfg
func (p *L5Influencer) SetAppConfig(cfg *util.AppConfig) {
	p.cfg = cfg
}

// GetDelta1 - get Delta1
func (p *L5Influencer) GetDelta1() int {
	return p.Delta1
}

// SetDelta1 - set Delta1
func (p *L5Influencer) SetDelta1(d int) {
	p.Delta1 = d
}

// GetDelta2 - get Delta2
func (p *L5Influencer) GetDelta2() int {
	return p.Delta2
}

// SetDelta2 - set Delta2
func (p *L5Influencer) SetDelta2(d int) {
	p.Delta2 = d
}

// GetDelta4 - get Delta4
func (p *L5Influencer) GetDelta4() int {
	return p.Delta4
}

// SetDelta4 - set Delta4
func (p *L5Influencer) SetDelta4(x int) {
	p.Delta4 = x
}

// SetID - set ID
func (p *L5Influencer) SetID() {
	p.ID = fmt.Sprintf("L5Influencer|%d|%d|%d|%s", p.Delta1, p.Delta2, p.Delta4, util.GenerateRefNo())
}

// Init - initializes a L5Influencer
func (p *L5Influencer) Init(i *Investor, cfg *util.AppConfig, delta4 int) {
	p.myInvestor = i
	p.cfg = cfg
	p.SetID()
	p.Delta4 = delta4
...
func (p *L5Influencer) Subclass() string {
	return "L5Influencer"
}
...
// ----------------------------------------------------------------------------
func (p *L5Influencer) DNA() string {
...
//	t2 = T3-p.Delta2
//
// val1 = L5_C1metric(t1)/L5_C2metric(t1)
// vaL5 = L5_C1metric(t2)/L5_C2metric(t2)
// delta = vaL5 - val1
...
//	vaL5       - an indicator value at T3 - p.Delta2 days
...
//
// ---------------------------------------------------------------------------
func (p *L5Influencer) GetPrediction(t3 time.Time) (string, float64, float64, float64, float64, error) {
...
// ------------------------------------------------------------------------------------
func (p *L5Influencer) CalculateFitnessScore() float64 {
