...
// LBInfluencer is the Influencer that predicts based on Linguistic Sentiment associated with a country
type LBInfluencer struct {
	cfg                 *util.AppConfig
	Delta1              int
	Delta2              int
...
// GetID - get ID string
func (p *LBInfluencer) GetID() string {
	return p.ID
}

// SetAppConfig - set cfg
func (p *LBInfluencer) SetAppConfig(cfg *util.AppConfig) {
	p.cfg = cfg
}

// GetDelta1 - get Delta1
func (p *LBInfluencer) GetDelta1() int {
	return p.Delta1
}

// SetDelta1 - set Delta1
func (p *LBInfluencer) SetDelta1(d int) {
	p.Delta1 = d
}

// GetDelta2 - get Delta2
func (p *LBInfluencer) GetDelta2() int {
	return p.Delta2
}

// SetDelta2 - set Delta2
func (p *LBInfluencer) SetDelta2(d int) {
	p.Delta2 = d
}

// SetID - set ID
func (p *LBInfluencer) SetID() {
	p.ID = fmt.Sprintf("LBInfluencer|%d|%d|%s", p.Delta1, p.Delta2, util.GenerateRefNo())
}

// Init - initializes a LBInfluencer
func (p *LBInfluencer) Init(i *Investor, cfg *util.AppConfig, delta4 int) {
	p.myInvestor = i
	p.cfg = cfg
	p.SetID()
	// p.flagpos = 5
...
func (p *LBInfluencer) Subclass() string {
	return "LBInfluencer"
}
...
// ----------------------------------------------------------------------------
func (p *LBInfluencer) DNA() string {
...
//	vaLB       - an indicator value at T3 - p.Delta2 days
...
//
// ---------------------------------------------------------------------------
func (p *LBInfluencer) GetPrediction(t3 time.Time) (string, float64, float64, float64, float64, error) {
...
// ------------------------------------------------------------------------------------
func (p *LBInfluencer) CalculateFitnessScore() float64 {
