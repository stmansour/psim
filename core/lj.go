...
// LJInfluencer is the Influencer that predicts based on Linguistic Sentiment associated with a country
type LJInfluencer struct {
	cfg                 *util.AppConfig
	Delta1              int
	Delta2              int
...
// GetID - get ID string
func (p *LJInfluencer) GetID() string {
	return p.ID
}

// SetAppConfig - set cfg
func (p *LJInfluencer) SetAppConfig(cfg *util.AppConfig) {
	p.cfg = cfg
}

// GetDelta1 - get Delta1
func (p *LJInfluencer) GetDelta1() int {
	return p.Delta1
}

// SetDelta1 - set Delta1
func (p *LJInfluencer) SetDelta1(d int) {
	p.Delta1 = d
}

// GetDelta2 - get Delta2
func (p *LJInfluencer) GetDelta2() int {
	return p.Delta2
}

// SetDelta2 - set Delta2
func (p *LJInfluencer) SetDelta2(d int) {
	p.Delta2 = d
}

// SetID - set ID
func (p *LJInfluencer) SetID() {
	p.ID = fmt.Sprintf("LJInfluencer|%d|%d|%s", p.Delta1, p.Delta2, util.GenerateRefNo())
}

// Init - initializes a LJInfluencer
func (p *LJInfluencer) Init(i *Investor, cfg *util.AppConfig, delta4 int) {
	p.myInvestor = i
	p.cfg = cfg
	p.SetID()
	// p.flagpos = 5
...
func (p *LJInfluencer) Subclass() string {
	return "LJInfluencer"
}
...
// ----------------------------------------------------------------------------
func (p *LJInfluencer) DNA() string {
...
//	vaLJ       - an indicator value at T3 - p.Delta2 days
...
//
// ---------------------------------------------------------------------------
func (p *LJInfluencer) GetPrediction(t3 time.Time) (string, float64, float64, float64, float64, error) {
...
// ------------------------------------------------------------------------------------
func (p *LJInfluencer) CalculateFitnessScore() float64 {
